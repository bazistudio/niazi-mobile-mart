...
import { SaleReturnableInfoDto, CreateSalesReturnLineDto } from '@/lib/tauri/tauriClient';
...
export const SalesReturnModal: React.FC<Props> = ({ saleId, onClose, onSuccess }) => {
  const [loading, setLoading] = useState<boolean>(true);
  const [submitting, setSubmitting] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [info, setInfo] = useState<SaleReturnableInfoDto | null>(null);

  // Return line selections: sale_line_id -> quantity
  ...
  useEffect(() => {
    async function loadReturnable() {
      try {
        setLoading(true);
        setError(null);
        const res = await returnsApi.getSaleReturnableInfo(saleId);
        setInfo(res);

        // Default to CASH, or CUSTOMER_CREDIT if customer exists
        if (res.customer_id) {
          setRefundMethod('CUSTOMER_CREDIT');
        } else {
        ...
        for (const line of res.lines) {
          initQty[line.sale_line_id] = 0;
        }
        setQuantities(initQty);
      } catch (err: any) {
        setError(err?.message || 'Failed to load sale return details');
      } finally {
        setLoading(false);
      }
    }
  ...
  // Calculate total return amount based on historical line prices
  const totalReturnAmount = (info?.lines || []).reduce((acc, line) => {
    const qty = quantities[line.sale_line_id] || 0;
    return acc + qty * line.original_unit_price;
  }, 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedLinesCount === 0) {
      setError('Please specify at least 1 item to return.');
      return;
    }

    const lines: CreateSalesReturnLineDto[] = Object.entries(quantities)
      .filter(([_, q]) => q > 0)
      .map(([sale_line_id, quantity]) => ({
        sale_line_id,
        quantity,
      }));
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[92vh] border border-gray-100 dark:border-gray-700">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-[#e76f51]/10 text-[#e76f51] rounded-xl">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                Return sold items to restore inventory and refund cash or credit balance.
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          {loading ? (
            <div className="py-12 text-center text-gray-500">Loading sale details...</div>
          ) : error && !info ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="bg-gray-50 dark:bg-gray-900/40 p-4 rounded-xl border border-gray-200 dark:border-gray-700 flex justify-between items-center text-sm">
                <div>
                  <span className="text-xs text-gray-400 block uppercase font-medium">Customer</span>
                  <span className="font-semibold text-gray-900 dark:text-white">
                    {info.customer_name || 'Walk-in Retail Customer'}
                  </span>
                </div>
                <div className="text-right">
                  <span className="text-xs text-gray-400 block uppercase font-medium">Original Invoice</span>
                  <span className="font-semibold text-gray-900 dark:text-white">{info.invoice_number}</span>
                </div>
              </div>
                  ...
                  <table className="w-full text-left text-sm">
                    <thead className="bg-gray-50 dark:bg-gray-900/60 border-b border-gray-200 dark:border-gray-700 text-xs text-gray-500 uppercase">
                      <tr>
                        ...
                        <th className="px-3 py-3 text-center">Already Ret.</th>
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                      {info.lines.map((l) => {
                        const maxQty = l.returnable_quantity;
                        const currentQty = quantities[l.sale_line_id] || 0;
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-3 py-3 text-center text-gray-500">{l.already_returned_quantity}</td>
                            <td className="px-3 py-3 text-center font-bold text-[#003049] dark:text-teal-400">
                              {l.returnable_quantity}
                            </td>
                            <td className="px-3 py-3 text-right">Rs {l.original_unit_price.toLocaleString()}</td>
                            <td className="px-4 py-3 text-right">
                              {isExhausted ? (
                                ...
                                <input
                                  ...
                                  placeholder="0"
                                  onChange={(e) => handleQtyChange(l.sale_line_id, maxQty, e.target.value)}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <button
                      type="button"
                      disabled={!info.customer_id}
                      onClick={() => setRefundMethod('CUSTOMER_CREDIT')}
                      className={`py-2 px-3 rounded-xl border text-sm font-medium flex items-center justify-center gap-2 transition-all ${
                        refundMethod === 'CUSTOMER_CREDIT'
                          ? 'border-[#006970] bg-[#006970]/10 text-[#006970] font-bold shadow-sm'
                          : 'border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50'
                      } ${!info.customer_id ? 'opacity-40 cursor-not-allowed' : ''}`}
                      title={!info.customer_id ? 'Only available for registered customers' : ''}
                    >
                  ...
                  {!info.customer_id && (
                    <span className="text-[11px] text-gray-400 mt-1 block">
                      Walk-in customer refunds must be settled via Cash.
                    </span>
                  )}
                </div>
              ...
              <div className="p-4 bg-gray-50 dark:bg-gray-900/60 rounded-xl border border-gray-200 dark:border-gray-700 flex justify-between items-center">
                <div>
                  <span className="text-xs text-gray-500 uppercase font-semibold">Total Historical Refund</span>
                  <div className="text-xs text-gray-400">
                    {selectedLinesCount} line item{selectedLinesCount !== 1 ? 's' : ''} selected
                  </div>
                </div>
                <div className="text-2xl font-black text-[#c1121f] dark:text-red-400">
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                <button
                  ...
                  className="px-6 py-2.5 bg-[#e76f51] hover:bg-[#d85d3f] text-white text-sm font-bold rounded-xl shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 transition-all cursor-pointer"
                >
                  {submitting ? (
                    'Processing Return...'
                  ) : (
