...
export function OrganizationSuppliersPage() {
  const [activeTab, setActiveTab] = useState('all');

  const supplierSections = [
    { id: 'all', label: 'All Suppliers', icon: Truck, desc: 'Central organization vendor directory, contact details, and vendor terms.' },
    { id: 'activity', label: 'Supplier Activity', icon: Activity, desc: 'Active procurement contracts, pending deliveries, and order fulfillment status.' },
    { id: 'purchases', label: 'Purchases', icon: ShoppingCart, desc: 'Enterprise purchase orders and bulk acquisition volumes.' },
    { id: 'payables', label: 'Payables', icon: ArrowUpRight, desc: 'Outstanding balances owed to suppliers and upcoming payment schedules.' },
    { id: 'history', label: 'Supplier History', icon: History, desc: 'Historical supply deliveries, purchase returns, and settlement records.' },
    { id: 'branches', label: 'Branch Activity', icon: Store, desc: 'Direct branch supply deliveries and local procurement allocations.' },
  ];

  const currentSection = supplierSections.find((s) => s.id === activeTab) || supplierSections[0];

  return (
    <OrganizationPageShell
      title="Organization Suppliers"
      description="Global supplier registry, procurement agreements, and accounts payable across all branches."
      badge="Vendor Registry"
      action={
        <button
          disabled
    ...
    >
      ...
      <div className="bg-white dark:bg-gray-800/90 rounded-xl p-4 border border-gray-200/80 dark:border-gray-700/80 shadow-sm flex flex-col md:flex-row items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-500">
          <span className="px-3 py-1.5 bg-gray-100 dark:bg-gray-700 rounded-md">Status: Active Suppliers</span>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800/90 rounded-xl p-12 border border-gray-200/80 dark:border-gray-700/80 shadow-sm text-center">
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-4 italic">
          Organization-level supplier directories and central procurement records will appear here.
        </p>
      </div>
    </OrganizationPageShell>
  );
...
export default OrganizationSuppliersPage;
